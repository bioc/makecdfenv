...
Let's say your CDF file is called \verb+elephantgenome.cdf+ and
is in your working directory. To make a package, simply write
\begin{Sinput}
R> make.cdf.package("elephantgenome.cdf")
\end{Sinput}
This will create a subdirectory \verb+elephantgenomecdf+ in your
...
\begin{Sinput}
$ R CMD INSTALL elephantgenomecdf   
\end{Sinput}
This will install the package into your R. You are now ready to use the 
affy package to process elephant genechips.
...
\begin{Sinput}
R> elephantcdf = make.cdf.env("elephantgenome.cdf")
\end{Sinput} 
...
\verb+elephantcdf+ with what ever the value of \verb+pname+ is.
